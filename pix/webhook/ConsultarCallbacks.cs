...
namespace Sdk.PixApi {
	public class ConsultarCallbacks {
		...
		public PaginaCallbacks GetPage(Config config, string dataHoraInicio, string dataHoraFim, int pagina, int tamanhoPagina, FiltroBuscarCallbacks filtro) {
			string url = $"{Constants.URL_PIX_WEBHOOK_PIX.Replace("AMBIENTE", config.Ambiente)}/{Constants.CALLBACKS}?dataHoraInicio={dataHoraInicio}&dataHoraFim={dataHoraFim}&paginaAtual={pagina}{(tamanhoPagina == 0 ? "" : "&itensPorPagina=" + tamanhoPagina)}{Addfilters(filtro)}";
